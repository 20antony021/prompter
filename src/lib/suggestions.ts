...
export async function generateTopicSuggestions(
  ...
  // try {
    const prompt = `<ROLE>
      You are a market analyst selecting brands for competitive SEO analysis in the \"Prompter\" tool.
    </ROLE>
    ...
    return result.object.suggestions;
  // } catch (error) {
    // Silently handle API errors - network issues, timeouts, etc.
    // This feature is non-critical and should degrade gracefully
  //   if (error instanceof Error && error.message.includes("Connect Timeout")) {
  //     console.warn("Google API timeout - suggestions unavailable (possible network/firewall issue)");
  //   } else {
