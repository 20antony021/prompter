...
export async function processUserMentions(userId: string, topicId?: string) {
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      // Delete all existing mentions for these prompts BEFORE processing batches
      // This fixes the concurrency bug where each batch was deleting all mentions
      console.log(`Deleting existing mentions for ${promptIds.length} prompts`);
      await tx
        .delete(mentions)
      ...
      let totalMentions = 0;
      const errors: string[] = [];
      const allBatchMentions: MentionInsert[] = [];
        ...
        batchJobs.push(
          queue.add(async () => {
            try {
              ...
              // Collect all mentions instead of inserting per batch
              allBatchMentions.push(...batchMentions);
      ...
      // Insert all collected mentions at once after all batches complete
      console.log(`Inserting ${allBatchMentions.length} total mentions`);
      const INSERT_CHUNK_SIZE = 100;
      for (
        let i = 0;
        i < allBatchMentions.length;
        i += INSERT_CHUNK_SIZE
      ) {
        const chunk = allBatchMentions.slice(i, i + INSERT_CHUNK_SIZE);
