...
import { getAvailableProviders } from "../subscription";
...
export async function processInBackground(
  ...
  try {
    // Get user's allowed providers based on their plan
    const allowedProviders = await getAvailableProviders();
    console.log(`Processing prompt ${promptId} with providers:`, allowedProviders);

    const [{ processPromptWithAllProviders }, { modelResults }] =
      await Promise.all([import("@/lib/llm"), import("@/db/schema")]);

    const results = await withTimeout(
      processPromptWithAllProviders(content, geoRegion, allowedProviders),
      180000,
