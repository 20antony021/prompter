...
export async function processPromptWithAllProviders(
  ...
  // If no providers specified, use all providers (backward compatibility)
  const providers = allowedProviders || ["openai", "claude", "google"];

  const promises: Promise<LLMResponse>[] = [];

  if (providers.includes("openai")) {
    promises.push(processPromptWithOpenAI(searchPrompt, region));
  }
  if (providers.includes("google")) {
    promises.push(processPromptWithGoogle(searchPrompt, region));
  }
  if (providers.includes("claude")) {
    promises.push(processPromptWithClaude(searchPrompt, region));
  }
